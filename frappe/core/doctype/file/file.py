...
class File(Document):
	...
	def save_file_on_filesystem(self):
		...
		bucket_name = 'goda-nms-dev'
		file_content = safe_file_name 

		if self.is_private:
			self.file_url = self.upload_to_s3(file_name, file_content)
		else:
			self.file_url = self.upload_to_s3(file_name, file_content)
			print(self.file_url,"safe_file_name gggg")
			self.file_url_down = self.download_from_s3(file_name)
			print(self.file_url_down,"file_url_down dfafaf")

		# fpath = self.write_file()

		return {"file_name": os.path.basename(self.file_url), "file_url": self.file_url}
		# return {"file_name": self.file_url, "file_url": self.file_url}
	...
	def upload_to_s3(self, file_name, file_content):
		...
		try:
			...
			print(response,"respose dasfadsf")
	...
	def download_from_s3(self, file_name):
		...
		try:
			...
			print(response['Body'].read(),"respose down")
		except Exception as e:
			print(f"Error downloading to S3: {e}")
			raise  
	...
	def delete_file_from_filesystem(self, only_thumbnail=False):
		...
		if only_thumbnail:
			...
			print(self.file_url,"delted file self.file_url")
